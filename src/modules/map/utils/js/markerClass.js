...
export const yellowIcon = (nro, classBlink) =>
	new L.divIcon({
		iconSize: [25, 41],
		iconAnchor: [10, 20],
		popupAnchor: [1, -34],
		...
		className: 'leaflet-marker-icon-green',
		html: `<div style="min-width: 3rem;min-height: 3rem;display: block;position: relative; color:black;font-size:15px;">
				<span class="marcador_ubicacion_map ${classBlink}" style="background: yellow;border: 1px solid #00000069;box-shadow: 3px 1px 3px #000000a1;" >
					<a class="icono_marcador_user_map" style="font-size: 14px;"></a>
				</span>
				${nro ? `<p class="sing">${nro}</p>` : ''}
				</div>`,
...
export const getIcon = (status, nro) => {
	switch (status) {
		...
		case 4:
			// Alerta
			return yellowIcon(nro, 'blink')
		case 5:
			// En Mantenimiento
			return workIcon()
		case 6:
			// Alerta en abierto
			return yellowIcon(nro, 'greenblink')
		case 7:
			// Alerta en abierto
			return yellowIcon(nro, 'grayblink')
		default:
