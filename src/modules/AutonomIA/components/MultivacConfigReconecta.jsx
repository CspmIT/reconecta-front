// Portado del Tablero Cooptech (módulo AutonomIA) a Reconecta — 09/2026.
// Cambios respecto del original: prop opcional `onGrabado` (inventario).
...
export default function MultivacConfigReconecta({ habilitado, conectado, enviarLinea, rxSink, terminal, log, selectorFirmware, botonesConexion, onGrabado }) {
  ...
  const grabar = async () => {
    ...
    try {
      ...
      // Reconecta: inventario de configuraciones (nunca frena el flujo).
      try { onGrabado?.({ comandos: comandos.length, mac: estado.mac || null, nombre: campos.nombre || null }); } catch { /* */ }
