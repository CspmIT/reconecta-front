...
function LoginApp() {
	...
	const onSubmit = async (data) => {
		try {
			if (recovery) {
				...
				storage.set('usuarioCooptech', { token: responseData.token, id_user: responseData.id, cliente: responseData.cliente[0] })
				if (responseData.cliente.length > 1) {
					...
					const product = await getProduct('Oficina Virtual', responseData.cliente[0].id, responseData.id)
	...
	return (
		<div className={`min-h-[100vh] w-full flex justify-center items-center bg-gray-300 bg-cover !bg-[url('src/views/LoginApp/assets/img/FondoGris.jpg')]`}>
					...
					{!recovery && (
						<TextField
							...
							{...register('password', {
								...
								pattern: { value: /^(?=.*[A-Z]).{8,}$/, message: 'El Formato correcto debe tener al menos 1 Mayuscula, 1 Simbolo especial y 1 Numero' },
							})}
							InputProps={{
								endAdornment: (
