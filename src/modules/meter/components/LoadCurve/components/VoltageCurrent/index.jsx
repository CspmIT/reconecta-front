...
import { getTableVoltageCurrent } from './utils/js/actions'
import { Button, TextField } from '@mui/material'
function VoltageCurrent({ info }) {
	const navigate = useNavigate()
	const [isLoading, setIsLoading] = useState(true)
	const [dataTable, setdataTable] = useState([])
	const getDataVoltageCurrent = async (dateStart = null, dateFinished = null) => {
		try {
			setIsLoading(true)
			const dataVoltageCurrent = await request(
				`${backend[`${import.meta.env.VITE_APP_NAME}`]}/getVoltageCurrent`,
				'POST',
				{
					serial: info.serial,
			...
			const VoltageCurrentData = await getTableVoltageCurrent(dataVoltageCurrent.data)
			setdataTable(VoltageCurrentData)
			setIsLoading(false)
		} catch (error) {
			console.error(error)
			Swal.fire({
	...
	useEffect(() => {
		if (!info) {
			...
			getDataVoltageCurrent()
		}
	}, [info])

	const filterTable = (data) => {
		getDataVoltageCurrent(data.dateStart, data.dateFinished)
	}
