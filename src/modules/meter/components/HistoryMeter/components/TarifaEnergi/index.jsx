...
function TarifaEnergi({ info }) {
	...
	// Validación cruzada: suma de registros por tarifa vs acumulado activa importada (IET_3)
	const validateTarifas = async (tarifas, body) => {
		try {
			const totales = await request(
				`${backend[`${import.meta.env.VITE_APP_NAME}`]}/getHistoryEnergyTotal`,
				'POST',
				body
			)
			const acumulado = parseFloat(totales.data?.IET_3?.value)
			const suma = ['RTE_0', 'RTE_1', 'RTE_2'].reduce(
				(acc, key) => acc + (parseFloat(tarifas[key]?.value) || 0),
				0
			)
			if (isNaN(acumulado) || !acumulado) {
				setValidator(null)
				return
			}
			const pct = (suma / acumulado) * 100
			setValidator({ suma, acumulado, pct, ok: pct >= 99 && pct <= 101 })
		} catch (error) {
			console.error(error)
			setValidator(null)
		}
	...
	const fmtNum = (num) => num.toLocaleString('es-AR', { maximumFractionDigits: 2 })
	return (
		<>
			{validator && (
				<div
					className={`w-full flex items-center gap-2 text-sm rounded-lg px-3.5 py-2.5 mb-4 border ${
						validator.ok
				...
				>
					<span>
						Validación cruzada: suma por tarifa <b>{fmtNum(validator.suma)} kWh</b> vs. acumulado
						activa <b>{fmtNum(validator.acumulado)} kWh</b> · <b>{validator.pct.toFixed(2)}%</b>{' '}
						{validator.ok ? '(coincide)' : '— revisar diferencia'}
					</span>
				</div>
			)}
