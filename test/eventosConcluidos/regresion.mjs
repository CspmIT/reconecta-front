...
for (const f of fs.readdirSync(dir).filter((n) => n.endsWith('.json') && !n.endsWith('.esperado.json'))) {
  ...
  const filas = fx.registros.map((r) => ({ ts: enUtc(r.fecha), id: r.idEvento, version: fx.version, equipo: fx.equipo, info: r.info }));
  const salida = procesar(filas, { gapSegundos: 2 }).map((e) => ({ inicio: e.inicio.toISOString(), codigo: e.codigo, etiquetas: e.etiquetas, severidad: e.severidad, categoria: e.categoria, veredicto: e.veredicto, detalle: e.detalle, crudos: e.crudos.length }));
